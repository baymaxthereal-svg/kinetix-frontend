...
export default function Orders() {
    ...
    // Filter orders by search term (name, id, phone, city)
    ...
    const OrderStatus = ({ isComplete }) => {
        if (isComplete) {
            return (
                <span className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-secondary-container text-on-secondary-container text-[11px] font-bold uppercase tracking-tight">
        ...
        return (
            <span className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-tertiary-fixed text-on-tertiary-fixed-variant text-[11px] font-bold uppercase tracking-tight">
    ...
    return (
        <main className="p-8 space-y-8">
            {/* Search Bar (only search, no filter/export buttons) */}
                    ...
                    <table className="w-full text-left border-collapse">
                        <thead>
                            <tr className="bg-surface-container-low">
                                <th className="px-6 py-5 text-[11px] font-bold text-on-surface-variant uppercase tracking-widest">Order ID</th>
                                ...
                                <th className="px-6 py-5 text-[11px] font-bold text-on-surface-variant uppercase tracking-widest">Total Amount</th>
                            ...
                            {filteredOrders.length === 0 ? (
                                ...
                                filteredOrders.map((order) => (
                                    <tr key={order.id} className="hover:bg-surface-bright transition-colors group">
                                        ...
                                        <td className="px-6 py-5 text-sm text-on-surface-variant">{formatDate(order.created_at)}</td>
                                        <td className="px-6 py-5">
                                            <div className="flex items-center gap-3">
                                                <div className="w-8 h-8 rounded-lg bg-secondary-fixed flex items-center justify-center text-on-secondary-fixed font-bold text-xs">
                                                    {getInitials(order.name)}
                                                </div>
                                                <div>
                                                    <p className="text-sm font-semibold text-on-surface">{order.name}</p>
                                                    <p className="text-[10px] text-on-surface-variant uppercase">{order.city}</p>
                                                </div>
                                            </div>
                                        </td>
                                        <td className="px-6 py-5 text-sm">{order.totalItems} unit{order.totalItems !== 1 ? 's' : ''}</td>
                                        <td className="px-6 py-5 text-sm font-bold text-on-surface">${order.totalAmount.toFixed(2)}</td>
                                        <td className="px-6 py-5">
